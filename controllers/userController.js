...
class UserController {
  ...
  async list(req, res) {
    try {
      ...
      const users = await User.find({}).select("-senha").limit(limit).skip(skip)
  ...
  async listOne(req, res) {
    try {
      const user = await User.findById(req.params.id).select("_id nome email createdAt updatedAt role approved")
      if (!user) {
  ...
  async listPendingApprovals(req, res) {
    try {
      const pendingUsers = await User.find({ approved: false }).select("-senha")
  ...
  async create(req, res) {
    try {
      ...
      const userRole = role || "PreAprovacao"
      let approved = false
  ...
  async updateObrasPermitidas(req, res) {
    try {
      ...
      if (req.userRole !== "Admin") {
        return res.status(403).json({
          error: true,
          message: "Somente Admin pode gerenciar permissões de obras!",
        })
      }
      ...
      // Atualiza as obras permitidas
      usuario.obrasPermitidas = obrasIds || []
      await usuario.save()

      return res.json({
        error: false,
        message: "Permissões de obras atualizadas com sucesso!",
        obrasPermitidas: usuario.obrasPermitidas,
      })
    } catch (error) {
      return res.status(500).json({ status: "error", msg: error.message })
    }
  ...
  async getObrasPermitidas(req, res) {
    try {
      const { userId } = req.params

      // Verifica se é Admin ou se está consultando suas próprias permissões
      ...
      const usuario = await User.findById(userId)
        .populate("obrasPermitidas", "nome cliente status")
        .select("nome email obrasPermitidas")
